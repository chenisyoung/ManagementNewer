...
--(
--	Class_name nvarchar(4) not null primary key
--)
--insert into [dbo].[Classes] values
--('班级一'),('班级二'),('班级三')
--不需要教学班,太复杂了,作业需求也没有 直接以行政班作为教学班
--drop table TeachingClass_number
--CREATE TABLE [dbo].[TeachingClass_number]  --创建教学班级主键约束表, 每门课程三个教学班级
--(
--	[Class_num] INT UNIQUE,		-- 唯一性
--	[Course_name] NVARCHAR (4) NOT NULL,
--	--定义 NVARCHAR 型，长度为 30 的教学班级列
--	[Class_name] NVARCHAR(4) not null, -- 班级名称
--	CONSTRAINT [PK_Class_num] PRIMARY KEY ([Class_name]),--设置教学班级列主键约束
--	CONSTRAINT [FK_Classes_Class_name] FOREIGN KEY ([Class_name]) references
--	[dbo].[Classes] (Class_name)
--	--设置班级外键约束
--)
--alter table TeachingClass_number drop constraint FK_Classes_Class_name
...
values('数信学院'),('电气工程学院'),('土木建筑工程学院'),('化学化工学院'),('马克思主义学院'),('计算机与电子信息学院') --输入学院信息
...
values('电气工程及其自动化'),('自动化'),('化学'),('土木工程'),('哲学'),('计算机类'),('机械类') --输入专业信息
...
values('学科必修课'),('通识必修课'),('学类核心课'),('学门核心课'),('专业选修课') --输入课程性质信息

CREATE TABLE [dbo].[Way_Of_Investigation] ( --创建考查方式主键约束表
[Way Of Investigation] NCHAR (4) NOT NULL,
--定义 NVARCHAR 型，长度为 4 的考查方式列
CONSTRAINT [PK_Way_Of_Investigation] PRIMARY KEY ([Way Of Investigation]) --设置考查方式为表的主键
);
...
values('西 3-101'),('西 3-102'),('西 3-103'),('西 3-106'),('西 3-105'),('西 3-104'),('西 3-107'),('西 3-108'),('西 3-109') --输入教室号信息
...
values('周一第一大节'),('周一第二大节'),('周一第三大节'),('周一第四大节'),(' 周一第五大节'), --输入上课时间信息
